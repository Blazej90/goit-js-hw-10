...
//         const image = `<img src="${cat.url}" alt="${name}" width="800px" height="600px">`;
...
function createCatContainer(image, info) {
  return `<div class="cat-container">${image}${info}</div>`;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    axios
      ...
      .then(response => {
        ...
        const info = `<h2>${name}</h2><p><strong>Description:</strong> ${description}</p><p><strong>Temperament:</strong> ${temperament}</p>`;
        const catContainer = createCatContainer(image, info);
        catInfo.innerHTML = catContainer;
